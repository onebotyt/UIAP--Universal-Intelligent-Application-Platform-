...
export class EventStore {
     ...
     */
    static async claimEvents(batchSize = 50) {
     ...
     */
    static async markFailed(id, error, maxAttempts = 4) {
        await transaction(async (client) => {
            const res = await client.query(`SELECT attempt_count FROM core.event_inbox WHERE id = $1 FOR UPDATE`, [id]);
            if (res.rowCount === 0)
                return;
     ...
     */
    static async markRejected(id, reason) {
        await query(`
      UPDATE core.event_inbox
      SET status = 'REJECTED',
          last_error = $2,
