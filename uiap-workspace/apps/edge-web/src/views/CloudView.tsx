    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--uiap-text-muted)' }}>Manage connection to the UIAP Developer Cloud</p>
        </div>
        <button className="uiap-btn" onClick={fetchStatus}>Refresh Status</button>
      </div>

      {error && (
        <div className="uiap-view-panel" style={{ color: 'var(--uiap-danger)', borderColor: 'var(--uiap-danger)' }}>
          {error}
        </div>
      )}
      ...
      <div className="uiap-view-panel" style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <span style={{ color: 'var(--uiap-text-muted)', fontFamily: 'monospace' }}>{status?.cloudUrl || 'Not set'}</span>
          </div>

          {!status?.configured && (
            <p style={{ color: 'var(--uiap-text-muted)', fontSize: '0.9em', marginTop: '10px' }}>
              To connect this Edge installation to the cloud, configure UIAP_CLOUD_URL and UIAP_INSTALL_KEY in your environment or .env file.
            </p>
          )}
        </div>
      </div>
      ...
      {status?.configured && (
        <div className="uiap-view-panel" style={{ marginBottom: '20px' }}>
          ...
          {status.entitlements.length === 0 ? (
            <p style={{ color: 'var(--uiap-text-muted)', textAlign: 'center' }}>No modules are currently licensed to this installation.</p>
          ) : (
            <table className="uiap-table" style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
              <thead>
              ...
              <tbody>
                {status.entitlements.map(ent => (
                  <tr key={ent.slug} style={{ borderBottom: '1px solid var(--uiap-border)' }}>
                    ...
                    <td style={{ padding: '10px 0', color: 'var(--uiap-text-muted)', fontFamily: 'monospace' }}>v{ent.version || 'unknown'}</td>
                    <td style={{ padding: '10px 0' }}>
                      <span style={{
                        ...
                        fontSize: '0.8em',
                        border: '1px solid',
                        borderColor: ent.status === 'active' ? 'var(--uiap-success)' : 'var(--uiap-text-muted)',
                        color: ent.status === 'active' ? 'var(--uiap-success)' : 'var(--uiap-text-muted)'
                      }}>
                        {ent.status}
                      </span>
                    </td>
      ...
      {status?.configured && (
        <div className="uiap-view-panel">
          ...
          <table className="uiap-table" style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {status.entitlements.map(ent => (
                <tr key={ent.slug + '-sync'} style={{ borderBottom: '1px solid var(--uiap-border)' }}>
                  <td style={{ padding: '10px 0', fontFamily: 'monospace' }}>{ent.slug}</td>
                  <td style={{ padding: '10px 0' }}>
                  ...
                  <td style={{ padding: '10px 0', color: ent.status === 'active' ? 'var(--uiap-success)' : 'var(--uiap-danger)' }}>
