...
dashboardRouter.get('/stats', requireAuth, async (_req: Request, res: Response) => {
  try {
    ...
    const enabledModules = await db('core_module_installations').count('id as cnt').where({ is_enabled: true }).first();
    const totalDevices = await db('core_device_registry').count('id as cnt').first();
    
    const failedEvents = await db('core_event_inbox').count('id as cnt').where({ status: 'FAILED' }).first();
    const pendingEvents = await db('core_event_inbox').count('id as cnt').where({ status: 'PENDING' }).first();
