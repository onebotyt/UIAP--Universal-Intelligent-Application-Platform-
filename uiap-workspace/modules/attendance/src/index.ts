...
export default class AttendanceModule implements UIAPModule {
  ...
  public async activate(context: ModuleContext): Promise<void> {
    ...
    this.unsubscribe = context.events.subscribe('attendance.checkin.request', async (event) => {
      context.logger.info(`Received attendance.checkin.request event: ${JSON.stringify(event.payload)}`);
      try {
        ...
        if (recent) {
           ...
           context.logger.info(`Duplicate protection: Student ${student_id} already checked in today at ${recent.check_in_time}`);
           return;
        }
    ...
    router.get('/records', context.rbac.require('attendance.records', 'view'), async (req, res) => {
      try {
        const db = context.db.getBuilder();
        const records = await db('attendance_records').orderBy('record_date', 'desc').orderBy('check_in_time', 'desc').limit(1000);
        ...
        const mapped = records.map((r: any) => ({
          ...
          date: r.record_date instanceof Date ? r.record_date.toISOString().split('T')[0] : String(r.record_date).split('T')[0],
    ...
    router.post('/records', context.rbac.require('attendance.records', 'manage'), async (req, res) => {
      try {
        const { studentId, className, subject, date, checkInTime, status, method, markedBy } = req.body;
        const db = context.db.getBuilder();
    ...
    // POST /records/batch
    router.post('/records/batch', context.rbac.require('attendance.records', 'manage'), async (req, res) => {
      try {
        ...
        await db.transaction(async (trx: any) => {
          for (const rec of records) {
            ...
            await trx('attendance_records').where({ student_id: rec.studentId, record_date: rec.date }).delete();
    ...
    // PUT /records/:id
    router.put('/records/:id', context.rbac.require('attendance.records', 'manage'), async (req, res) => {
      try {
        const { id } = req.params;
    ...
    // DELETE /records/:id
    router.delete('/records/:id', context.rbac.require('attendance.records', 'manage'), async (req, res) => {
      try {
        const { id } = req.params;
        const db = context.db.getBuilder();
    ...
    router.delete('/records', context.rbac.require('attendance.records', 'manage'), async (req, res) => {
      try {
